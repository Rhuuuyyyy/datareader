...
TipoCategoria string_para_categoria(const char* nome_categoria) {
    ...
    if (strcmp(nome_categoria, "Bebidas (alcoólicas e não alcoólicas)") == 0 ||
    ...
    if (strcmp(nome_categoria, "Gorduras e óleos") == 0)
    ...
    if (strcmp(nome_categoria, "Miscelâneas") == 0)
        return MISCELANEAS;
    ...
    if (strcmp(nome_categoria, "Produtos açucarados") == 0)
        return PRODUTOS_ACUCARADOS;
    if (strcmp(nome_categoria, "Verduras, hortaliças e derivados") == 0)
